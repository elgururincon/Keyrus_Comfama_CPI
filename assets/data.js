...
const FLOWS = [
...

  {
    ...
    role: "query",
    type: "Servicio · Consulta",
    description: "API de consulta de Cases en Salesforce con datos enriquecidos: el caso, sus comentarios (CaseComment), sus adjuntos directos (ContentDocumentLink + ContentVersion) y los correos asociados (EmailMessage) con los adjuntos de cada correo embebidos como base64.",
    endpoint: { method: "POST", path: "/v2/casos/consulta", protocol: "HTTPS" },
    lines: 3071,
    participants: 9,
    scripts: 11,
    subprocesses: ["Consulta Adjuntos", "Consulta Comentarios", "Consulta Emails", "Get Token"],
    mappings: ["Mapeo.mmap"],
    receivers: [
      { name: "Salesforce OAuth Token", method: "POST", auth: "Basic (SF_User_Keyrus)" },
      { name: "Salesforce Case Query", method: "GET" },
      { name: "Salesforce CaseComment Query", method: "GET" },
      { name: "Salesforce ContentDocumentLink Query (Case)", method: "GET" },
      { name: "Salesforce EmailMessage Query", method: "GET" },
      { name: "Salesforce CDL + ContentVersion + VersionData (per email)", method: "GET (dinámico)" },
    ],
    logic: [
      "Recibe filtros (CaseNumber, AccountId, fechas, toggle 'emails', etc.) y construye SOQL dinámico con ORDER BY CreatedDate DESC.",
      "Query 1: trae los Cases. Extrae IDs y mapeo caseId → CaseNumber.",
      "Query 2 (paralela): CaseComment por caseId.",
      "Query 3 (paralela): ContentDocumentLink + ContentVersion para adjuntos directos del caso.",
      "Query 4 (NUEVA): EmailMessage por ParentId, con TextBody, From, To, Subject, MessageDate, Status. Se omite si el toggle 'emails' viene en false o no hay caseIds.",
      "Para cada EmailMessage: consulta su propio CDL → ContentVersion → descarga VersionData como base64 y embebe los adjuntos dentro del correo.",
      "merge_attachments fusiona los 4 datasets en una respuesta jerárquica: case → { comentarios, adjuntos, emails: [{ ..., Adjuntos:[] }] }.",
    ],
    techniques: ["ExternalCall (×5)", "Script (×11)", "ProcessCallElement (×4)", "Mapping", "HTTP GET dinámico desde Groovy"],
    highlight: true,
  },
];
...
const SCRIPTS_BY_FLOW = {
  ...
  "Consulta_de_casos": [
    { name: "filtros.groovy",                purpose: "Arma SOQL principal para los Cases con filtros recibidos. Soporta toggle 'emails' para activar/desactivar la consulta de correos. Añade ORDER BY CreatedDate DESC. (Modificado en v4)" },
    { name: "build_cdl_query.groovy",        purpose: "Construye SOQL de ContentDocumentLink filtrado por los caseIds extraídos (adjuntos del caso)." },
    { name: "build_comments_query.groovy",   purpose: "Construye SOQL de CaseComment filtrado por los caseIds extraídos." },
    { name: "build_cv_query.groovy",         purpose: "Construye SOQL de ContentVersion para traer base64 de los adjuntos del caso." },
    { name: "script3.groovy (build_email_query)", purpose: "NUEVO en v4 — Construye SOQL de EmailMessage por ParentId. Setea flag skipEmails si no hay caseIds." },
    { name: "script4.groovy (procesar_emails)",   purpose: "NUEVO en v4 — Pieza más compleja (~170 líneas): agrupa EmailMessage por Case, consulta el CDL de cada email, obtiene el ContentVersion Id, descarga VersionData como base64 desde Salesforce y embebe los adjuntos dentro de cada correo. Usa helpers httpGetJson y httpGetBase64." },
    { name: "merge_attachments.groovy",      purpose: "Fusiona los 4 resultsets (Cases + Comments + Adjuntos + Emails) en una estructura jerárquica por caso. (Modificado en v4: ahora también consume emailsBody)" },
    ...
    { name: "Clean_Bad_Request.groovy",      purpose: "Sub-proceso de error: devuelve mensaje legible cuando los filtros son inválidos." },
  ],
};
